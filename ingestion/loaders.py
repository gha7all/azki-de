...
def _normalize_row(r) -> Tuple[int, str, str, str, str]:
    # users.csv columns: user_id, signup_date, city, device_type
    user_id = int(r.user_id)
    signup_date = str(getattr(r, "signup_date", ""))
    city = str(getattr(r, "city", ""))
    device_type = str(getattr(r, "device_type", ""))
...
def load_users_snapshot(csv_path: str, batch_size: int = 1000) -> int:
    """
    loads users into clickhouse users table and validates if it's needed.
    the batch size controls how many rows are inserted at once.
    """
    ...
    # Ensure expected columns exist (defaults if missing)
    ...
    rows = (_normalize_row(r) for r in df.itertuples(index=False))
...
if __name__ == "__main__":
    ...
    path = sys.argv[1]
    count = load_users_snapshot(path)
